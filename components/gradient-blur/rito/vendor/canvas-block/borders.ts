...
function renderRoundedBorders(
  ...
  const geometry = resolveRoundedBorderGeometry(
    borders,
    x,
    y,
    width,
...
function renderCrescentRoundedBorder(
  ...
  if (!edges.every((edge) => edge.style === "solid" && edge.color === color))
    return false;
  ...
  try {
    ...
    traceInnerEllipticalPath(
      ...
      [
        ...
        [
          Math.max(0, radiusX - right.width),
          Math.max(0, radiusY - bottom.width),
        ],
        [
          Math.max(0, radiusX - left.width),
          Math.max(0, radiusY - bottom.width),
        ],
      ],
    );
...
function traceInnerEllipticalPath(
  ...
  const cap = (value: number, limit: number) =>
    Math.min(Math.max(0, value), limit / 2);
