...
export function renderBackgroundImage(
  ...
  const image = resolveImageGeometry(
    bitmap,
    background,
    rect.x,
    rect.y,
...
function resolveImageGeometry(
  ...
  const { drawW, drawH } = resolveImageSize(
    size,
    bitmap.width,
    bitmap.height,
    boxW,
  ...
  const position =
    background.position ??
    (size === "auto" ? DEFAULT_POS_AUTO : DEFAULT_POS_CENTER);
  return {
    bitmap,
...
function resolveImageSize(
  ...
  if (typeof size === "object") {
    ...
    const drawW =
      explicitW ??
      (explicitH !== undefined
        ? (explicitH * imageWidth) / imageHeight
        : imageWidth);
    const drawH =
      explicitH ??
      (explicitW !== undefined
        ? (explicitW * imageHeight) / imageWidth
        : imageHeight);
    return { drawW, drawH };
  }
...
function drawRepeatedImage(
  ...
  const startX =
    image.drawX - Math.ceil((image.drawX - blockX) / image.drawW) * image.drawW;
  const startY =
    image.drawY - Math.ceil((image.drawY - blockY) / image.drawH) * image.drawH;
...
function resolvePositionAxis(
  ...
  if (value.unit === "percent")
    return ((containerSize - imageSize) * value.value) / 100;
  return value.value;
}
