...
export function drawCanvasTextFragment(
  ...
  if (
    textRidesTheLayoutGrid(paint.font.sizePx, paint.wordSpacingPx, drawnText)
  ) {
    drawTextOnLayoutGrid(
      ctx,
      drawnText,
      penX,
      baseline,
...
export function drawCanvasRubyFragment(
  ...
  try {
    ...
    const expands =
      glyphs > 1 && free > 0.01 && rubyAnnotationExpands(ruby.text);
    const words: string[] = [];
    {
      let start = -1;
    ...
    if (
      ...
      const share =
        (ruby.rect.width - naturalWords - naturalSpaces) / words.length;
      ...
      const x =
        Math.floor(
          (ruby.rect.x + (ruby.rect.width - measured.width) / 2) * 64,
        ) / 64;
...
function effectiveTextColor(
  ...
  // DOM computed styles already own the foreground/background pairing.
  void colorOverride;
  void declaredGround;
