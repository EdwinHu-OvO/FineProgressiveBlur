...
export function drawTextShadows(
  ...
  const rotated =
    Math.abs(transform.b || 0) > 1e-6 || Math.abs(transform.c || 0) > 1e-6;
  if (!rotated) {
...
function syncTextState(
  ...
  destination.letterSpacing = canvasSpacingValue(
    fragment.paint.letterSpacingPx,
  );
}
