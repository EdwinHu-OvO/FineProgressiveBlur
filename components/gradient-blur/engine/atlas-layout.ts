import type { GradientBlurBandMetrics } from "../types";
...
function levelEnd(maxRadius: number, minimumRadius: number): number {
  ...
  for (let iteration = 0; iteration < 24; iteration += 1) {
    ...
    if (blurRadiusAt(middle, maxRadius) >= minimumRadius) low = middle;
    else high = middle;
  }
  return low;
}
...
export function createAtlasLayout(
  ...
  for (const level of LEVELS) {
    const end = levelEnd(radius * pixelRatio, level.minimumRadius);
    ...
    const blendEnd = levelEnd(radius * pixelRatio, level.minimumRadius * 0.75);
    ...
    const halo = Math.ceil(3 * blurRadiusAt(start, radius) * pixelRatio);
